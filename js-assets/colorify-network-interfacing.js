...
function writePost(archive, postSubmission) {
  ...
  postSubmission.addEventListener("submit", function(e) {
    ...
    var formSignatureImage =
      formRecieved.getElementsByTagName("img").length > 0
        ? formRecieved.getElementsByTagName("img")[0].src
        : "not available";
    ...
    async function postFile(archive, postContent) {
      await archive.writeFile(
        "/posts/post-" + postContent.timestamp + ".json",
...
function userAndTheirPosts(userCounter, userList, watchingContainer) {
  ...
  loadProfile(userUrl)
    .then(function(userInfo) {
      ...
      watchingContainer.insertAdjacentHTML(
        "beforeend",
        `
      <li>
            <hr />
      ...
      loadPosts(userArchive)
        .then(function(userPosts) {
          ...
          userPosts.posts.forEach(function(post) {
            loadPostContent(userUrl, post).then(function(postAndArchive) {
              ...
              if (JSON.stringify(post.content).includes('src=\\"')) {
                thisPostContent = JSON.stringify(thisPostContent).replace(
                  'src=\\"',
                  'src="' + postAndArchive.archive.url + "/"
                );
              ...
              userPostContainer.insertAdjacentHTML(
                "beforeend",
                `
            <li>
            <hr/>
            ...
            `
              ...
              if (
                postCounter >= amountOfPosts &&
                userCounter < userAmount - 1
              ) {
                //foreach appending posts for this user is over
                userCounter++;
