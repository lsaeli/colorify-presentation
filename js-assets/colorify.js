...
window.onload = function() {
  ...
  function checkColors(listColor) {
    // console.log(colors);
    filteredColors = [];
    colors.forEach(function(color) {
      if (checkifSelected(listColor, color)) {
  ...
  //a list of all the colorContainers so we can insert things into them
  let selections = [
    { container: "colorContainerRed", color: "#ff0000" },
    { container: "colorContainerPink", color: "#ffc0cb" },
    { container: "colorContainerOrange", color: "#ffa500" },
    { container: "colorContainerYellow", color: "#ffff00" },
    { container: "colorContainerGreen", color: "#008000" },
    { container: "colorContainerBlue", color: "#0000ff" },
    { container: "colorContainerPurple", color: "#800080" },
    { container: "colorContainerBrown", color: "#a52a2a" },
    { container: "colorContainerBlack", color: "#000000" },
    { container: "colorContainerWhite", color: "#ffffff" },
    { container: "colorContainerNone", color: "none" }
  ];

  // Get the target and color and generate a template literal of all the colors that match to be injected back into the page
  function initiateOverlayPosts(target, linkColor) {
    if (linkColor !== undefined || linkColor !== "none" || target !== "none") {
      var colorsList = checkColors(linkColor);
      colorsDisplay = "";
      colorsList.forEach(function(color) {
        colorsDisplay =
          colorsDisplay +
          `<a  id="hexColorLink" style="color:${color};">${color}</a>`;
      });
  ...
  function initiateLoadPosts(archive) {
    ...
    loadPosts(archive).then(function(userPosts) {
      var totalPostLength = userPosts.posts.length;
      var i = 0;
      userPosts.posts.forEach(function(post) {
        loadPostContent(archive, post).then(function(postAndArchive) {
          ...
          if (postAndArchive.post.colors !== undefined) {
            var shouldIdisplay = checkifSelected(
              selectedColor,
              postAndArchive.post.colors
            );
          }

          //shortcut if statement
          let showColors =
            postAndArchive.post.colors == undefined ? "hidden" : "colors";
          let showEmoticon =
            postAndArchive.post.emoticon == undefined ? "hidden" : "emoticon";
          let signatureImg =
            postAndArchive.post.signature == undefined ? "hidden" : "signature";

          //The variable to show and hide posts that we apply to the html
          let showPosts = shouldIdisplay ? "show" : "hide";

          // if a color exists on this post, add it to the colors array
          if (showColors == "colors") {
            colors.push(postAndArchive.post.colors);
          }

          // the magic loop that looks through the list of the overlay colors, and inserts the links with the correct colors back into the page.
          // only goes once all the posts have loaded
          i++;
          if (i > totalPostLength - 1) {
            selections.forEach(function(obj) {
              initiateOverlayPosts(obj.container, obj.color);
            });
          }

          var signature =
            postAndArchive.post.signature != undefined &&
            postAndArchive.post.signature != null
              ? postAndArchive.post.signature
              : "";

          var displayImage = signature.includes("data:image")
            ? ""
            : "display: none;";
        ...
        <div id="postBackground" class=${showPosts}> 
              <div id="postText" >
                          <h2 id="postHeader" style="color:${
                            postAndArchive.post.colors
                          };">${postAndArchive.post.title}</h2>
                          <h4 id="timestamp">${
                            postAndArchive.post.timestamp
                          }</h4>
                          ...
                          <div class=displaySignature> 
                            ...
                            <p id="postPara">${
                              postAndArchive.post.content
                            } <br></p>

                              <div id="hexColorIcon" style="background-color:${
                                postAndArchive.post.colors
                              };"></div>


                            <div id="postEmoticon">
                                <p class=${showEmoticon} style="color:${
              postAndArchive.post.colors
            };">${postAndArchive.post.emoticon}</p>
                            </div>

                            <div id="hexColor">
                                <p class=${showColors} style="color:${
              postAndArchive.post.colors
            };">${postAndArchive.post.colors}</p>
                            </div>
